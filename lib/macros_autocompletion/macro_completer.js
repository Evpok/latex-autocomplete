...
// Previous line incompatible with node < 8.0.0 so here's a workaround
    ...
    getSuggestions: async function(request) {
        ...
        while (match = magicMacro.exec(text)) {
            ...
            if (file) {
                ...
                matching_macros.push(...find_macros(lu.strip_comments(text), escape_regex(macro_name_prefix)))
            }
        }
